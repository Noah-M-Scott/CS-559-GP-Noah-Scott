...
let sandMat = new T.MeshBasicMaterial({
  color: "lime",
  side: T.DoubleSide,
  wireframe: true
});
...
//        }
//    });
let sand_obj = new T.Mesh(new T.SphereGeometry( 24.5, 16, 8 ), sandMat);
...
const maxBubbles = 16;
const bubble_obj = new T.Mesh(new T.SphereGeometry(1.0, 16, 8), goldMat);
...
const maxUrchin = 16;
//urchin
let urchin_obj = new T.Mesh(new T.BoxGeometry(1.0, 1.0, 1.0));
//urchin_obj.traverse(obj => {
//    if (obj instanceof T.Mesh) {
		urchin_obj.castShadow = true;
		urchin_obj.scale.set(2.0, 2.0, 2.0);
		for (let i = 0; i < maxUrchin; i++) {
			urchins.push(urchin_obj.clone());
...
const maxFish = 17;
// fish head
let fishhead_obj = new T.Mesh(new T.BoxGeometry(1.0, 1.5, 1.0));
//fishhead_obj.traverse(obj => {
//    if (obj instanceof T.Mesh) {
		fishhead_obj.castShadow = true;
		for (let i = 0; i < maxFish; i++) {
			fish.push(fishhead_obj.clone());
			fish[i].material = fishMat.clone();				
			fish[i].position.set(0, 0, 0.125); 
			fishSize.push(1.0);
...
// fish body
let fishbody_obj = new T.Mesh(new T.BoxGeometry(1.0, 2.0, 1.0));
//fishbody_obj.traverse(obj => {
//    if (obj instanceof T.Mesh) {
		fishbody_obj.castShadow = true;
		for (let i = 0; i < maxFish; i++) {
			const k = fishbody_obj.clone();
...
// fish tail
let fishtail_obj = new T.Mesh(new T.BoxGeometry(1.0, 1.5, 1.0));
//fishtail_obj.traverse(obj => {
//    if (obj instanceof T.Mesh) {
		fishtail_obj.castShadow = true;
		for (let i = 0; i < maxFish; i++) {
			const k = fishtail_obj.clone();
			k.material = fish[i].material;				
